...
class PatchTST_MoE_cluster_time(nn.Module):
    def __init__(self, bs: int, c_in: int, c_out: int, context_window: int, target_window: int, patch_len: int, stride: int,
        ...
        # Cluster
        # Cluster gating uses the input variable dimension (c_in) to match the
        # flattened shape passed into EDESC: [bs * patch_num_out, nvars * d_model].
        self.cluster = EDESC(d_model=d_model, n_clusters=T_num_expert, eta=eta, c_in=c_in, bs=bs,
