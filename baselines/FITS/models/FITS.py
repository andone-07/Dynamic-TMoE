...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # Calculate target frequency dimension and ensure upsampled size doesn't exceed it
        self.target_freq = int((self.seq_len + self.pred_len) / 2 + 1)
        self.upsampled_freq = min(int(self.dominance_freq * self.length_ratio), self.target_freq)

        if self.individual:
            self.freq_upsampler = nn.ModuleList()
            for i in range(self.channels):
                self.freq_upsampler.append(nn.Linear(self.dominance_freq, self.upsampled_freq).to(torch.cfloat))

        else:
            self.freq_upsampler = nn.Linear(self.dominance_freq, self.upsampled_freq).to(torch.cfloat) # complex layer for frequency upcampling]
    ...
    def forward(self, x):
        ...
        if self.individual:
            low_specxy_ = torch.zeros([low_specx.size(0), self.upsampled_freq, low_specx.size(2)],dtype=low_specx.dtype).to(low_specx.device)
            for i in range(self.channels):
        ...
        # print(low_specxy_)
        low_specxy = torch.zeros([low_specxy_.size(0), self.target_freq, low_specxy_.size(2)],dtype=low_specxy_.dtype).to(low_specxy_.device)
